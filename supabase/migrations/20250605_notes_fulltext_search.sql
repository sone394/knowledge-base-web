-- 笔记查询性能优化 + 全文搜索（在 Supabase SQL Editor 中执行本脚本）
...
-- ---------------------------------------------------------------------------
-- 3. 全文搜索 RPC（受 RLS 约束，仅搜索当前用户未删除笔记）
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.search_notes(text) IS
  '按标题与正文全文搜索当前用户的笔记，按 ts_rank 排序';
